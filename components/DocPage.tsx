          ...
          <div className="glass p-6 md:p-10">
            <div className="text-center mb-8">
              ...
              <span className="inline-block mb-3">
                <span
                  className="text-sm font-bold uppercase tracking-widest border-[3px] border-black px-4 py-1"
                  style={{ background: color, color: '#000', boxShadow: '3px 3px 0 0 #000' }}
                >
                  {badge}
                </span>
              </span>
              ...
              <ul className="space-y-2">
                {capabilities.map((c, i) => (
                  <li key={i} className="flex items-start gap-3 text-gray-700 dark:text-gray-200">
                    <span className="mt-1 w-3 h-3 flex-none" style={{ background: color, border: '2px solid #000' }} />
                    <span>{c}</span>
                  </li>
                ))}
              </ul>
            ...
            {(examples?.length || visualExamples?.length) && (
              <section className="mb-8">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  {examples?.map((ex) => (
                    <div key={ex.fileUrl} className="glass-soft p-4">
                      <div className="flex items-center justify-between gap-3 mb-3">
                        ...
                        <a
                          ...
                          className="brute-btn shrink-0 px-3 py-1.5 text-xs font-bold text-black"
                          style={{ background: color, border: '3px solid #000', boxShadow: '3px 3px 0 0 #000' }}
                        >
                          Unduh
                        </a>
                      </div>
                      <iframe
                        ...
                        className="w-full h-64 border-2 border-black bg-white"
                      />
                    </div>
                  ))}
                  {visualExamples?.map((ve) => (
                    <div key={ve.title} className="glass-soft p-4">
                      <p className="font-semibold text-gray-800 dark:text-gray-100">{ve.title}</p>
                      <p className="text-xs text-gray-500 mb-3">{ve.note}</p>
                      <div className="border-2 border-black overflow-hidden">{ve.view}</div>
                    </div>
                  ))}
                </div>
              </section>
                    ...
                    <motion.div key={section.title} whileHover={{ scale: 1.008 }} transition={{ type: 'spring', stiffness: 300, damping: 24 }}>
                      ...
                      >
                        ...
                        <motion.span
                          ...
                          className="brute-btn flex items-center justify-center w-7 h-7 text-sm text-black"
                          style={{ background: open ? color : 'rgba(255,255,255,0.12)', border: '2px solid #000', boxShadow: open ? '2px 2px 0 0 #000' : 'none' }}
                        >
                          ▾
                        </motion.span>
                      </button>
            ...
            <div className="mt-8 pt-6 border-t-2 border-black flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link
                href={toolHref}
                className="brute-btn px-8 py-3 font-semibold text-black"
